...
This preserves the detailed component notes and validation history from the
original README. Dated results and status descriptions below are historical;
they do not establish the current completion of the study. In particular, early
statements that the full model had never run predate the later H100 checks and
pilots. Software validation is separate from research findings and human review.

Run all commands from the **repository root**; inline paths use that directory.
Saved example requests may require artifacts under the ignored `.cache/` directory
and their original source versions. Links to parent MATS records are available
only in the full local workspace, not in a standalone clone of this repository.
